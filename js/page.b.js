...
<div class="pagination" id=${id}>
    ...
    Page.addPageTurning = function(currentPage, prevbutton, nextbutton, cpInput, eleTotal, theTotalPages, theids, outputEle, funcsendid, funcresulttoele){
       funcresulttoele=funcresulttoele||Page.resulthtml;
       ...
       function changePage(page) {
           ...
           funcsendid(Page.getTenItemsAsString(theids, page), outputEle, funcresulttoele);
       }
    ...
    Page.getarticles = function(idsString, element, funcresulttoele){
           ...
           success: function(rjson) {
               ...
               for(let id of idsString.split(",")){
                   for (let i of r.articles) {
                       if(i.id != id){continue;}
                       element.innerHTML += funcresulttoele(i);
                   }
               }
           }
       });
    };
    //用ids获取workshop fileinfo id，载入到element中
    Page.getworkshops = function(idsString, element, funcresulttoele){
       if(typeof idsString !== "string"){
            idsString = Page.getTenItemsAsString(idsString);
       }
       ajax({
           url: '/php/savefile/savefiles.php',
           method: "POST",
           send: `ids=${idsString}`,
           success: function(rjson) {
               let r = JSON.parse(rjson);
    ...
    Page.gettopics = function(idsString, element, funcresulttoele){
       if(typeof idsString !== "string"){
            idsString = Page.getTenItemsAsString(idsString);
       }
       ajax({
           url: '/php/forum/topics.php',
           method: "POST",
           send: `ids=${idsString}`,
           success: function(rjson) {
               let r = JSON.parse(rjson);
    ...
    Page.resulthtml = function(i, isblank = false) {
        ...
        return `
            <a href="../article/?id=${i.id}" target="${target}">
    ...
    Page.resulthtml2 = function(i, isblank = false) {
        let target = isblank ? "_blank" : "_self";
        return `
            <a href="../workshop/?id=${i.id}" target="${target}">
            <div class="${Page.data.resultblock ? Page.data.resultblock : "contentBlock"}">
              <img src="${i.imgLink}" alt="预览图片" class="previewimg">
              <h3 class="workshop_title">
              ${i.title}
              </h3>
              <p>${i.content}</p>
            </div></a>
    ...
    Page.resulthtml3 = function(i, isblank = false) {
        let target = isblank ? "_blank" : "_self";
        return `
            <a href="../forum/?id=${i.id}" target="${target}">
            <div class="${Page.data.resultblock ? Page.data.resultblock : "contentBlock"}">
              <img src="${i.imgLink}" alt="预览图片" class="previewimg">
              <h3 class="workshop_title">
              ${i.title}
    ...
    //workshop的简单载入
    Page.aptEasy2 = function(pe, theids){
        Page.addPageTurning(1, pe.querySelector(".prevPage"), pe.querySelector(".nextPage"), pe.querySelector("input.currentPage"), pe.querySelector("span.totalPages"), Page.getPage(theids.length), theids, pe.querySelector(".output"), Page.getworkshops,Page.resulthtml2);
    }
    
    Page.aptEasy3 = function(pe, theids){
        Page.addPageTurning(1, pe.querySelector(".prevPage"), pe.querySelector(".nextPage"), pe.querySelector("input.currentPage"), pe.querySelector("span.totalPages"), Page.getPage(theids.length), theids, pe.querySelector(".output"), Page.gettopics,Page.resulthtml3);
    }
    return Page;
})();