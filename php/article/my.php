...
try{
    ...
    if(isset($_POST["keyword"])){
        ...
        $stmt=$conn->prepare("SELECT id FROM article WHERE (title LIKE ? OR content LIKE ?) AND user = {$usersign->uid}");
        ...
        $result=$conn->query("SELECT id FROM article WHERE user = {$usersign->uid}");
        if ($result->num_rows === 0) {
            throw new ProcessError(Constants::TEXT_NO_RESULT);
        }
        // 输出数据
