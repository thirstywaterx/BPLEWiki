...
try{
    ...
    $sql="SELECT user,tocomment FROM workshopcomment 
    WHERE id='{$acid}'";
    ...
    while ($queue) {
        ...
        $result = $conn->query("SELECT id FROM workshopcomment WHERE tocomment='{$current}'");
        while ($row = $result->fetch_assoc()) {
            array_puch($dids,$row["id"]);
            array_puch($queue,$row["id"]);
        }
    }
    $idstr=implode(',',$dids);
    $sql="DELETE FROM workshopcomment 
    WHERE id IN ({$idstr})";
    if(!$conn->query($sql)){
